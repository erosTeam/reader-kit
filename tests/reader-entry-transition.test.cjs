...
function scenario() {
  ...
  const target = () => {
    ...
    result.slotId = 7; result.assetRequestId = 3; result.navigationRevision = state.navigationRevision
...
test('stale request and wrong original page cannot replace the published target', () => {
  ...
  for (const change of [t => t.assetRequestId++, t => t.slotId++, t => t.selectionId++,
