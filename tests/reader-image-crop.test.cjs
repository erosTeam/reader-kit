...
test('paged crop is opt-in, reversible, asset-scoped and never changes topology', async () => {
  ...
  assert.equal(before.cropEnabled, false)
  session.setCropEnabled(true); await tick()
  let after = session.snapshot()
  assert.equal(after.cropEnabled, true)
  assert.equal(after.forItem(after.displayKeys[0]).cropEnabled, true)
  ...
  assert.equal(session.snapshot().cropEnabled, false)
