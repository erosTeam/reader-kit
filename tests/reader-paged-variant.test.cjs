...
test('resolved host variant preserves page identity and changes only after preparation', async () => {
  ...
  const preparing = f.s.snapshot()
  assert.equal(preparing.variantPreparing, true)
  assert.equal(preparing.isPreparingVariant(0, 'enhanced'), true)
  assert.equal(preparing.isPreparingVariant(0, 'translated'), false)
  assert.equal(preparing.isPreparingVariant(1, 'enhanced'), false)
  assert.equal(preparing.forItem(preparing.displayKeys[0]).isPreparingVariant(0, 'enhanced'), true)
  ...
  assert.equal(f.s.snapshot().isPreparingVariant(0, 'enhanced'), false)
